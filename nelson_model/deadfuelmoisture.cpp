...
//------------------------------------------------------------------------------
/*! \brief Determines the volume-weighted mean moisture content of the
    stick's outer radial profile.
  
    \return The volume-weighted mean moisture content of the stick's outer radial
    profile (g/g).
 */

double Sem::DeadFuelMoisture::outerMeanMoisture( void ) const
{
    double wbr = 0.0;
    for ( int i=0; i<(m_nodes/3)+1; i++ )
    {
        wbr += m_w[i] * (m_v[i]/.5775);
    }
    wbr = ( wbr > m_wmx ) ? m_wmx : wbr;
...
double Sem::DeadFuelMoisture::medianRadialMoisture(void) const
{
	vector<double> vMw;
	for (int i = 0; i<m_nodes; i++)
	{
